...
extension UIColor {
    ...
    //MARK: HEX
    func convertHexToRGB(_ hex:UInt32) -> UIColor {
        return self.changeHexColorCodeToRGB(hex, alpha: 1)
    }
    
    func convertHexStringToColor (_ hexString: String) -> UIColor {
        var hexColorString = hexString.trimmingCharacters(in:NSCharacterSet.whitespacesAndNewlines).uppercased()
        
        if hexColorString.hasPrefix("#") {
            hexColorString = hexColorString.substring(from: hexColorString.characters.index(hexColorString.startIndex, offsetBy: 1))
        }
    ...
    fileprivate
    func changeHexColorCodeToRGB(_ hex:UInt32, alpha: CGFloat) -> UIColor {
    ...
    func rgbCalculation(_ redColor:CGFloat, greenColor: CGFloat, blueColor: CGFloat, alphaValue:CGFloat) -> UIColor {
